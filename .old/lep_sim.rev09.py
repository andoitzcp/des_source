# -*- coding: utf-8 -*-
"""
Created on Mon Aug 22 00:33:47 2022
...
class lab_model:
    ...
    def generate_cv_st_qced_arrivals(self):
        while self.csqa<g.cv_st_qced_req:
            ...
            self.item_gen = cv(self.cv_counter, "QCED", "ST", "CV162")
        
            sampled_interarrival = random.expovariate(1.0/g.cv_st_qced_inter)
        
            yield self.env.timeout(sampled_interarrival) & self.indoor_inv_store.put(self.item_gen)
            
            print("inv ", self.indoor_inv_store.items)
            print(self.env.now)
    ...
    def rim_proc(self):
        while True:
            ...
            with self.butler_mach.request() as req_butler:
                ...
                self.item_rim = yield self.indoor_inv_store.get()
                
                print("rim ", self.indoor_rim_store.items)
            ...
            yield self.indoor_rim_store.put(self.item_rim)
    
            df_to_add_rim = pd.DataFrame({"CV_ID": [self.item_gen.id],
                                          "CV_gen_t": [cv_gen_t],
            ...
            with self.indoor_mach.request() as req_indoor:
                ...
                self.item_test_1 = yield self.indoor_rim_store.get()
                self.item_test_2 = yield self.indoor_rim_store.get()
                ...
                with self.technicians.request(priority=-3, preempt=False) as req_tech:
            ...
            df_to_add_indoor = pd.DataFrame({"CV_ID": [self.item_gen.id],
            ...
            self.results_df_indoor = pd.concat(
                [self.results_df_indoor, df_to_add_indoor])
    
    def generate_master_df(self):
    
            df_merged_master = pd.merge(
                self.results_df_rim, self.results_df_indoor, on="CV_ID")
            df_merged_master["CV_rim_q_d"] = df_merged_master["CV_rim_start_t"] - \
                df_merged_master["CV_gen_t"]
            df_merged_master["CV_rim_proc_d"] = df_merged_master["CV_rim_end_t"] - \
                df_merged_master["CV_rim_start_t"]
            df_merged_master["CV_mount_q_d"] = df_merged_master["CV_mount_start_t"] - \
                df_merged_master["CV_rim_end_t"]
            df_merged_master["CV_mount_proc_d"] = df_merged_master["CV_mount_end_t"] - \
                df_merged_master["CV_mount_start_t"]
            df_merged_master["CV_test_q_d"] = df_merged_master["CV_test_start_t"] - \
                df_merged_master["CV_mount_end_t"]
            df_merged_master["CV_test_proc_d"] = df_merged_master["CV_test_end_t"] - \
                df_merged_master["CV_test_start_t"]
