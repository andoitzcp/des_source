# -*- coding: utf-8 -*-
"""
Created on Tue Aug 30 15:05:46 2022
...
class lab_model:
    def __init__(self, run_number):
        ...
        self.cv_mount_1 = cv("initial", "QCED", "initial", "initial")
        self.cv_mount_2 = cv("initial", "QCED", "initial", "initial")
        ...
        self.results_df_cond = pd.DataFrame()
        self.results_df_cond["CV_ID"] = []
        self.results_df_cond["CV_cond_start_t"] = []
        self.results_df_cond["CV_cond_end_t"] = []
        self.results_df_cond.set_index("CV_ID", inplace=True)
    ...
    #Tire generator process for ST QCED tests
    def generate_cv_st_qced_arrivals(self):
        while self.csqa < g.cv_st_qced_req: #Keep generating tires until requierement is fullfilled
        
            self.cv_counter += 1
            
            self.csqa += 1
        
            self.cv_gen = cv(self.cv_counter, "QCED", "ST", "CV162")    #Create an instance of the cv class
    ...
    def obstruct_technician(self):
        while True:
            ...
            with self.technicians.request(priority=-4, preempt=True) as req_tech:
    ...
    def timeout_to_shift_end(self, time_left):
        
        time_left_shift_end = 8 - self.env.now % 24

        if time_left_shift_end < 0:
            return 0
        else:
            if time_left < time_left_shift_end:
                return time_left
            else:
                return time_left_shift_end
            ...
            if len(self.cv_gen_list) < 1:
                ...
                with self.indoor_rims.request() as req_indoor_rim:
                    ...
                    if self.indoor_split_counter % 2 == 0:
                        ...
                        try:
                            with self.indoor_mach.request() as indoor_req:
                                
                                yield indoor_req
                                
                                yield self.env.process(self.mount_proc(self.cv_mount_1, self.cv_mount_2))
                                
                                self.cv_mount_1 = self.cv_rim_list.pop(0)
                                self.cv_mount_2 = self.cv_rim_list.pop(0)
                                self.cv_mount_list.append(self.cv_mount_1)
                                self.cv_mount_list.append(self.cv_mount_2)
    ...
    def rim_proc(self, cv_rim):
        ...
        time_left_rim = random.triangular(1, 2, 3)
        
        while time_left_rim > 0:
            timeout = self.timeout_to_shift_end(time_left_rim)
        
            with self.technicians.request(priority=prio, preempt=False) as req_tech_rim, self.butler_mach.request() as req_butler:
                
                try:
                    
                    yield req_tech_rim & req_butler
                    
                    start.append(self.env.now)
                    
                    
                    
                    print("timeout : ", time_left_rim)
                    ...
                    time_left_rim -= self.env.now - start[-1]+0.01                    
                    
                    print("time letf rim : ", time_left_rim)
                
                except simpy.Interrupt:
                    time_left_rim -= self.env.now - start[-1]
                    
                    print("ex tlr ",time_left_rim)
                    # prio -= 0.05
                    pass
         
        print(len(start))
        cv_gen_t = start[0]
        cv_rim_start_t = start[1]    
        cv_rim_end_t = self.env.now
        #print("START : ", cv_rim_start_t, " END : ", cv_rim_end_t)
    ...
    def mount_proc(self, cv_mount_1, cv_mount_2):
        ...
        while time_left_mount > 0:
            ...
            timeout = self.timeout_to_shift_end(time_left_mount)
            
            with self.technicians.request(priority=-1, preempt=False) as req_tech_mount:
                
                try:
                    yield req_tech_mount
                    ...
                    time_left_mount -= self.env.now - start[-1] + 0.01
                    
                except simpy.Interrupt:
                    time_left_mount -= self.env.now - start[-1]
                    pass
        
        cv_mount_start_t = start[1]          
        
        cv_mount_end_t = self.env.now
            
        df_to_add_indoor_1 = pd.DataFrame({"CV_ID":[self.cv_mount_1.id],
                              "CV_mount_start_t":[cv_mount_start_t],
                              "CV_mount_end_t":[cv_mount_end_t]})
        df_to_add_indoor_1.set_index("CV_ID", inplace=True)
        df_to_add_indoor_2 = pd.DataFrame({"CV_ID":[self.cv_mount_2.id],
                              "CV_mount_start_t":[cv_mount_start_t],
                              "CV_mount_end_t":[cv_mount_end_t]})
    ...
    def cond_proc(self, cv_mount_1, cv_mount_2):
        
        cv_cond_start_t = self.env.now
            
        yield self.env.timeout(3)
        
        cv_cond_end_t = self.env.now
        
        df_to_add_cond_1 = pd.DataFrame({"CV_ID":[self.cv_mount_1.id],
                              "CV_cond_start_t":[cv_cond_start_t],
                              "CV_cond_end_t":[cv_cond_end_t]})
        df_to_add_cond_1.set_index("CV_ID", inplace=True)
        df_to_add_cond_2 = pd.DataFrame({"CV_ID":[self.cv_mount_2.id],
                              "CV_cond_start_t":[cv_cond_start_t],
                              "CV_cond_end_t":[cv_cond_end_t]})
        df_to_add_cond_2.set_index("CV_ID", inplace=True)
        self.results_df_cond = pd.concat([self.results_df_cond, df_to_add_cond_1, df_to_add_cond_2])
    ...
    def generate_master_df(self):
        ...
        df_merged_master = pd.merge(self.results_df_rim, self.results_df_indoor, self. results_df_cond, on="CV_ID")
