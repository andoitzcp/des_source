# -*- coding: utf-8 -*-
"""
Created on Tue Aug 16 17:31:27 2022
...
class lab_model:
    ...
    def generate_cv_st_qced_arrivals(self):
        while self.csqa<g.cv_st_qced_req:
            ...
            self.env.process(self.rim_proc(self.csq))
    ...
    def rim_proc(self, cubierta):
        ...
        req_tech = self.technicians.request(priority=0, preempt=False)
        ...
        sampled_rim_duration = random.triangular(12/60, 25/60, 15/60)
        
        yield self.env.timeout(sampled_rim_duration)
        ...
        self.env.process(self.indoor_proc(self.csq))
        
    def indoor_proc(self, cubierta):
        
        req_indoor = self.indoor_mach.request()
        req_tech = self.technicians.request(priority=-1, preempt=False)
